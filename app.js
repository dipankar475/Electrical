  ...
  Object.entries(SECTIONS).forEach(([secId, sec]) => {
    sec.clauses.forEach(cl => {
      ...
      rows += `<tr>
        <td>${cl.ref}</td>
        <td>${sec.label}</td>
        <td>${cl.desc.replace(/</g,'&lt;').replace(/>/g,'&gt;')}</td>
        <td>${cl.std}</td>
  ...
  Object.entries(SECTIONS).forEach(([secId, sec]) => {
    sec.clauses.forEach(cl => {
      ...
      if (sel && (sel.value === 'NC' || sel.value === 'OBS')) {
        ncs.push({ ...cl, sec: sec.label, status: sel.value, remark: rem ? rem.value : '' });
      }
    });
  });

  let rows = ncs.map(n => `<tr><td>${n.ref}</td><td>${n.sec}</td><td>${n.desc}</td><td>${n.std}</td><td style="text-transform:capitalize">${n.sev}</td><td style="font-weight:bold;color:${n.status==='NC'?'red':'orange'}">${n.status}</td><td>${n.remark}</td></tr>`).join('');
  const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>NC Report</title>
  <style>body{font-family:Arial,sans-serif;font-size:11px;padding:20px;}h1{color:#cc0000;font-size:16px;}table{width:100%;border-collapse:collapse;}th{background:#0a0e17;color:white;padding:7px;font-size:9px;}td{border:1px solid #ddd;padding:6px;}</style>
